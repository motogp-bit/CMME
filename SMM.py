...
def QQ_Modular_Multiplier(n: int, N: int, m: int = None) -> QuantumCircuit:
    ...
    P = QuantumRegister(n + 1, 'P')  # Product register (computational basis output)
    w = QuantumRegister(m, 'w')      # Fractional carry register (initially |0>)
    anc = QuantumRegister(1, 'anc')  # Ancilla for QQM internal carries
    
    qc = QuantumCircuit(x, y, P, w, anc)
    qc.append(QQM(n, N), [*x, *y, *w, anc])
    qc.append(QFT(num_qubits=n + 1, do_swaps=True).to_gate(), [*P])
    
    for j in range(m):          
        for i in range(n + 1):  
            # equivalent to adding N * 2^(i + j) / 2^(m + n + 1)
            phase = 2 * np.pi * N * (2**(i + j - m - n - 1))
            qc.cp(phase, w[j], P[i])
    ...
    for i in range(n + 1): 
        for j in range(m):  
            phase = -2 * np.pi * (2**(i + j - m)) / N
            qc.cp(phase, P[i], w[j])
            
    # 3.3 Inverse QFT on w to restore it cleanly back to state |0>
